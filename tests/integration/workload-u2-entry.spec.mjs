...
test('U3: deployed task entry remains on the production engine and reports the rebuilt runner version', async ({ page }) => {
  await page.goto('/');
  ...
  const jobs = [
    ...
    { params: base.params, strategy: base.strategy, mode: 'dsl', overrides: { ...overrides,
      unified: true, acceptance: { window: { hardCutoff: 0 } }, workloadModelVersion: 'workload-u3-64-tiered-v1' } },
  ...
  expect(results[2].replay.configuration.workloadModelVersion).not.toBe('workload-u3-64-tiered-v1');
